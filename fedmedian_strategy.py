...
class FedMedianStrategy(Strategy):
    ...
    def aggregate_fit(
        ...
        median_params = [
            np.median(np.stack([p[i] for p in params_list], axis=0), axis=0)
            for i in range(len(params_list[0]))
        ]
        updated_params = ndarrays_to_parameters(median_params)
        ...
        print(f"Round {server_round}: Training aggregated from {len(results)} clients (FedMedian)")
    ...
    def aggregate_evaluate(
        ...
        metrics_avg = {key: float(np.mean([float(v) for v in values])) for key, values in metrics_aggregated.items()}
        metrics_avg["round"] = server_round
        print(f"Round {server_round}: Evaluation aggregated from {len(results)} clients (FedMedian)")
        return weighted_loss, metrics_avg

    def evaluate(self, server_round: int, parameters: Parameters) -> Optional[Tuple[float, Dict[str, Scalar]]]:
        print(f"Round {server_round}: Evaluating global model (FedMedian)")
        return None
